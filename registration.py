...
def speak(text):
    ...
    # Use run to wait for audio to finish slightly, prevents overlapping crashes
    try:
...
def video_feed():
    def generate():
        while True:
            ...
            with frame_lock:
                if video_frame is not None:
                    # Encode here while holding lock to ensure data consistency
...
def logic_loop():
    ...
    while True:
        ...
        if detected == "Unknown":
            ...
            # --- CAPTURE ---
            speak("Hold still.")
            time.sleep(1.0) # Ensure stability
            ...
            # --- INPUT PHASE (PAUSE VISION TO PREVENT SEGFAULT) ---
            scan_active = False # CRITICAL: Stop heavy processing
            time.sleep(0.5)     # Allow vision thread to finish current cycle
            ...
            if pending_registration_data:
                ...
                scan_active = True # Resume vision to see them leave
...
def vision_loop():
    ...
    while True:
        ...
        # Only do heavy Dlib stuff if allowed
        ...
        # Draw box regardless of scan state (use last known location if paused)
        if current_face_location:
            ...
            if current_detected_name:
                cv2.putText(frame, str(current_detected_name), (left, top-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)

        # Thread-safe update
        # We copy frame *before* lock to minimize lock time, though read() already provides a unique buffer usually
